...
lint: _flake8 _isort _black _dead-fixtures _mypy  ## Check code lint
...
load-fixtures:  ## Load database random data for testing purposes from a given quantity (Eg: make load-fixtures quantity=50)
	poetry run python -m tests.load_fixtures $(quantity)
